...
/** Rodrigues rotation: angles in radians, world axes. */
...
function solvePose(a,bump,rack,seed) {
  ...
  let normal1,normal2;
  if(a.topology==='macpherson') { const axis=unit(sub(p.strut_top,p.strut_bottom)); normal1=unit(cross(axis,[0,1,0])); normal2=unit(cross(axis,normal1)); }
  const residual=q=> {
    const result=pairs.map(([i,o],n)=>norm(sub(transform(p[o],q),add(p[i],i==='tie_rod_inner'?[rack,0,0]:[0,0,0])))-lengths[n]);
...
function actuation(a,pose) {
  ...
  const position=angle=>add(pivot,rotate(rod,[0,angle,0]));
  ...
  out.rocker_rod=position(angle);out.rocker_damper=add(pivot,rotate(damper,[0,angle,0]));
...
export function solveCorner(a,bump=0,rack=0) {
  ...
  const axis=rotate([1,0,0],pose.rotation), degrees=180/Math.PI;
  return {...pose,camber:Math.atan2(-axis[2],axis[0])*degrees,toe:Math.atan2(axis[1],axis[0])*degrees,compression:actuator.compression};
}
export function solveAxle(a,bump=0,rack=0) {
  // Right local lateral coordinate is reversed; the rack translates in global +X.
  const left=solveCorner(a,bump,rack),right=solveCorner(a,bump,-rack);
