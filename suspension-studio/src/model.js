/** Canonical schema. Coordinates: X lateral-left, Y longitudinal, Z up; mm. */
...
export function newDesign() {
  return { version: 1, name: 'Untitled suspension', wheelbase: 1600, wheelDiameter: 460, axles: { front: axle(), rear: axle('double-wishbone', 1180, false) } };
}
...
export function generatePoints(a, diameter) {
  const t = a.track / 2, z = diameter / 2;
  const p = {
    lca_front: [t * .4, -125, z - 105], lca_rear: [t * .4, 125, z - 105], lca_outer: [t - 35, 0, z - 95],
    tie_rod_inner: [a.rackLength / 2, -95, z - 35], tie_rod_outer: [t - 25, -90, z - 35], wheel_center: [t, 0, z],
  };
  if (a.topology === 'double-wishbone') Object.assign(p, { uca_front: [t * .43, -110, z + 100], uca_rear: [t * .43, 110, z + 100], uca_outer: [t - 60, 0, z + 85] });
  if (a.topology === 'macpherson') Object.assign(p, { strut_top: [t * .66, 0, z + 320], strut_bottom: [t - 55, 0, z + 60] });
  if (a.topology === 'multi-link') {
    delete p.lca_front; delete p.lca_rear; delete p.lca_outer;
    Object.assign(p, { lower_front_inner: [t * .4, -170, z - 105], lower_front_outer: [t - 35, -40, z - 95], lower_rear_inner: [t * .45, 180, z - 95], lower_rear_outer: [t - 30, 35, z - 90], upper_front_inner: [t * .43, -150, z + 100], upper_front_outer: [t - 65, -30, z + 85], upper_rear_inner: [t * .47, 160, z + 85], upper_rear_outer: [t - 60, 35, z + 75] });
  }
  if (a.topology !== 'macpherson') {
    const pull = a.actuation === 'pullrod';
    p.actuation_outer = [t - 80, 0, pull ? z + 65 : z - 65];
    if (a.actuation === 'direct') p.damper_top = [t * .6, 0, z + 285];
    else Object.assign(p, { rocker_pivot: [t * .42, 0, pull ? z - 110 : z + 180], rocker_rod: [t * .42, 0, pull ? z - 35 : z + 105], rocker_damper: [t * .42 - 65, 0, pull ? z - 110 : z + 180], damper_top: [t * .18, 0, pull ? z + 140 : z + 300] });
  }
  return p;
}
...
export function changeDimensions(a, key, value) {
  ...
  if (key === 'track') for (const p of Object.values(a.hardpoints)) p[0] *= value / old;
  if (key === 'track') a.rackLength = a.hardpoints.tie_rod_inner[0] * 2;
  if (key === 'rackLength') a.hardpoints.tie_rod_inner[0] = value / 2;
}
export function validateDesign(d) {
  ...
  if (!d || d.version !== 1 || !d.axles) return ['Unsupported design file. Expected Suspension Studio schema version 1.'];
  ...
  for (const key of ['front','rear']) {
    const a = d.axles[key]; if (!a || !TOPOLOGIES[a.topology]) { errors.push(`${key}: invalid topology.`); continue; }
    ...
    if (wc && Math.abs(wc[0] * 2 - a.track) > .1) errors.push(`${key}: wheel center X must equal half the track width.`);
    if (rack && Math.abs(rack[0] * 2 - a.rackLength) > .1) errors.push(`${key}: rack inner X must equal half the rack length.`);
...
export function preset(id) {
  ...
  a.hardpoints = {...generatePoints(a,d.wheelDiameter), uca_front:[263.53,-127,263.53],uca_rear:[232.43,127,248.77],uca_outer:[482.6,9.12,285.98],lca_front:[215.9,-117.48,120.65],lca_rear:[215.9,123.42,127],lca_outer:[533.4,-3.18,119.13],tie_rod_inner:[219.08,-69.85,151.99],tie_rod_outer:[542.93,-73.03,171.45],wheel_center:[558.8,0,203.2]};
